...
function CartComponent() {
  ...
  const getCartList = async () => {
    const cartList = await getCartsDetails();
    const bookList = cartList.map((cartBook: any) => {
      return {
        ...books.filter((book: any) => book._id === cartBook.product_id._id)[0],
        cartId: cartBook._id,
  ...
  return (
    <div className="w-full h-full flex justify-center">
      {!dataloaded ? (
        <div className="w-[80%] font-[Roboto]">
          ...
          <div className="w-[80%] mt-5 min-h-[250px] border-[#707070] border">
            ...
            <div className="flex flex-col gap-2 p-5">
              ...
              <div
                className={
                  cartItems.length && !expanded ? "flex justify-end" : "hidden"
                }
              >
                <Button
                  variant="contained"
              ...
              <div>
                ...
                <div className="w-[80%] flex justify-between">
                  <div className="w-[48%] flex flex-col">
                    ...
                    <span className="px-5 py-2 h-[45px] border-2">
                ...
                {cartItems[0]?.user_id?.address?.map(
                  (useraddress: any, index: number) => (
                    <AddressCard useraddress={useraddress} index={index} />
                  )
                )}
                <div className={!expanded2 ? "flex justify-end" : "hidden"}>
                  <Button
        ...
        <CircularProgress className="m-20" />
      )}
    </div>
  );
}
